...
`;
  ...
  var ProductMedia = class extends CustomHTMLElement {
    async connectedCallback() {
      ...
      this.deskPictures = document.querySelectorAll("[bullet-picture]");
    ...
    async _setListeners() {
      for (const pic of this.deskPictures) {
      // this.deskPictures.forEach((pic,index) => {
        console.log("bullet pict initialized", pic);
        pic.addEventListener("click", async () => {
          let id = pic.getAttribute("media-id-custom");
          console.log("pic clicked", id);
          if (this.zoomButton && id) {
            if( this.first === true) {
              this.first = false;
              const unhide = document.querySelector(".hide-mobile-wrapper-cc1");
              console.log("unhide", unhide);
              unhide.style.overflow = "auto";
              console.log("zoom?1", pic, id);
              await this.mainCarousel.select(`[data-media-id="${id}"]`)
              this.zoomButton.click();
              unhide.style.overflow = "hidden";
            } else {
              console.log("zoom?", pic, id);
              await this.mainCarousel.select(`[data-media-id="${id}"]`)
              this.zoomButton.click();
            }
          }
